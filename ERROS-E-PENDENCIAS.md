...
|---|---|---|---|
| Alta | CORS aceita origens arbitrárias com credenciais | Uma origem externa pode tentar usar a sessão do navegador | Substituir o CORS refletido por uma lista explícita de domínios oficiais e testar origem permitida e negada |
...
| Alta | Testes externos dependem diretamente de Apps Script e Resend | `pnpm test` pode falhar por timeout mesmo quando o código está correto | Separar testes locais determinísticos dos testes externos com flag explícita, retry e timeout controlado |
...
| Baixa | Existem dois avisos de lint no painel administrativo | Indica código não utilizado e aumenta a manutenção | Remover `Alert` e `updateTeacher` ou criar a tela que os utiliza |
