import { ActivityIndicator, FlatList, Linking, Modal, Pressable, Text, TextInput, View } from "react-native";
...
export default function AdminScreen() {
  ...
  const provisionSpreadsheet = trpc.admin.provisionClassSpreadsheet.useMutation();
  ...
  const retrySpreadsheet = async (classId: number) => {
    try { await provisionSpreadsheet.mutateAsync({ classId }); setNotice("Planilha da turma criada."); await refresh(); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Não foi possível criar a planilha."); }
  };
  ...
  return <ScreenContainer className="bg-background" edges={["top", "left", "right"]}>
    <FlatList data={classes.data || []} keyExtractor={(item) => String(item.id)} contentContainerStyle={{ padding: 20, paddingBottom: 110 }} ListHeaderComponent={<View><View className="rounded-[28px] bg-foreground p-5"><Text className="text-[11px] font-black uppercase tracking-[1.8px] text-[#FFB15C]">Coordenação</Text><Text className="mt-2 text-2xl font-black text-white">Administração</Text><Text className="mt-2 text-sm leading-5 text-[#C7D5E3]">Contas, turmas e acesso às planilhas em um só lugar.</Text></View>{notice ? <Text className="mt-4 rounded-xl bg-[#E8F6EE] p-3 text-xs leading-5 text-[#176E46]">{notice}</Text> : null}<View className="mt-5 flex-row gap-3"><View className="flex-1"><Button label="Novo professor" onPress={() => setMode("teacher")} /></View><View className="flex-1"><Button label="Nova turma" secondary onPress={() => setMode("class")} /></View></View><Text className="mt-7 text-base font-black text-foreground">Turmas cadastradas</Text></View>} renderItem={({ item }) => <View className="mt-3 rounded-2xl border border-border bg-white p-4 shadow-sm"><View className="flex-row items-start justify-between"><View className="flex-1 pr-4"><Text className="text-base font-bold text-foreground">{item.name}</Text><Text className="mt-1 text-xs text-muted">{item.code} · {item.course || "Sem curso"} · {item.active ? "Ativa" : "Arquivada"}</Text><Text className="mt-1 text-xs text-muted">{item.teachers.length ? item.teachers.map((teacher) => teacher?.name || teacher?.email).join(", ") : "Sem professor vinculado"}</Text>{item.spreadsheetUrl ? <Pressable onPress={() => Linking.openURL(item.spreadsheetUrl!)}><Text className="mt-3 text-xs font-bold text-primary">Abrir planilha Google Sheets</Text></Pressable> : <Pressable onPress={() => retrySpreadsheet(item.id)} disabled={provisionSpreadsheet.isPending}><Text className="mt-3 text-xs font-bold text-[#B7791F]">{provisionSpreadsheet.isPending ? "Criando planilha..." : "Criar/re tentar planilha"}</Text></Pressable>}</View><Text className="text-xl text-primary">›</Text></View></View>} ListEmptyComponent={classes.isLoading ? <ActivityIndicator color="#F59A3D" className="mt-8" /> : <Text className="mt-4 text-sm text-muted">Ainda não há turmas cadastradas.</Text>} />
